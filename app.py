...
COLOR_TABLE_PATH = APP_DIR / "internal_color_hex_table_full_rows_by_fabric_code.csv"
...
def load_internal_color_table() -> list[dict[str, str]]:
    if not COLOR_TABLE_PATH.exists():
        return []
    with COLOR_TABLE_PATH.open("r", encoding="utf-8-sig", newline="") as fh:
        reader = csv.DictReader(fh)
        return [{str(k): ("" if v is None else str(v).strip()) for k, v in row.items()} for row in reader]


def internal_color_hex_value(row: dict[str, str]) -> str:
    for key in ("html????", "html颜色代码", "html", "HEX", "hex"):
...
def internal_color_option_label(row: dict[str, str]) -> str:
    fabric_code = row.get("成品面料内部编号", "").strip()
    base_code = row.get("基础面料编号", "").strip()
    color_code = row.get("颜色内部编号", "").strip()
    hex_value = internal_color_hex_value(row)
    parts = [part for part in [fabric_code, color_code, hex_value] if part]
    summary = " | ".join(parts) if parts else "未命名颜色"
    return f"{summary} ({base_code})" if base_code else summary


def internal_color_select_label(row: dict[str, str]) -> str:
    fabric_code = row.get("鎴愬搧闈㈡枡鍐呴儴缂栧彿", "").strip()
    color_code = row.get("棰滆壊鍐呴儴缂栧彿", "").strip()
    hex_value = internal_color_hex_value(row)
    parts = [part for part in [fabric_code, color_code, hex_value] if part]
    return " | ".join(parts) if parts else "鏈懡鍚嶉鑹?"


def unique_internal_color_rows() -> list[dict[str, str]]:
    unique_rows: list[dict[str, str]] = []
    seen_codes: set[str] = set()
    for row in load_internal_color_table():
        fabric_code = row.get("鎴愬搧闈㈡枡鍐呴儴缂栧彿", "").strip()
        if not fabric_code or fabric_code in seen_codes:
            continue
        seen_codes.add(fabric_code)
        unique_rows.append(row)
    return unique_rows


def color_table_row_value(row: dict[str, str], *keys: str) -> str:
    for key in keys:
        value = row.get(key, "").strip()
        if value:
            return value
...
def color_table_value_by_index(row: dict[str, str], index: int) -> str:
    keys = list(row.keys())
    if 0 <= index < len(keys):
        return row.get(keys[index], "").strip()
    return ""


def color_table_fabric_code(row: dict[str, str]) -> str:
    return color_table_value_by_index(row, 1) or color_table_row_value(row, "鎴愬搧闈㈡枡鍐呴儴缂栧彿")


def color_table_base_code(row: dict[str, str]) -> str:
    return color_table_value_by_index(row, 2) or color_table_row_value(row, "鍩虹闈㈡枡缂栧彿")


def color_table_color_code(row: dict[str, str]) -> str:
    return color_table_value_by_index(row, 3) or color_table_row_value(row, "棰滆壊鍐呴儴缂栧彿")


def color_table_validation_url(row: dict[str, str]) -> str:
    return color_table_value_by_index(row, 4) or color_table_row_value(row, "淇浘鍙傝€冨浘閾炬帴鍦板潃")


def unique_color_table_rows() -> list[dict[str, str]]:
    unique_rows: list[dict[str, str]] = []
    seen_codes: set[str] = set()
    for row in load_internal_color_table():
        fabric_code = color_table_fabric_code(row)
        if not fabric_code or fabric_code in seen_codes:
            continue
        seen_codes.add(fabric_code)
        unique_rows.append(row)
...
@st.cache_data(show_spinner=False)
def fetch_reference_image_from_url(url: str) -> np.ndarray | None:
    if not url:
        return None
    try:
...
def build_single_job_ui() -> None:
    ...
    color_table_rows = unique_color_table_rows()
    color_table_map = {color_table_fabric_code(row): row for row in color_table_rows}
    color_table_options = sorted([key for key in color_table_map.keys() if key])
        ...
        with ref_cols[idx]:
            ...
            if source_mode == "本地样例" and ref_name_options:
                ...
                source_choice = st.radio(
                    ...
                    ["手动上传", "成品面料编号"],
                    horizontal=True,
                    key=f"stable_color_source_choice_{idx}",
                    label_visibility="collapsed",
                )
                if source_choice == "成品面料编号" and color_table_options:
                    selected_code = st.selectbox(
                        f"{label_default} 成品面料编号",
                        color_table_options,
                        key=f"stable_color_table_select_{idx}",
                    )
                    selected_row = color_table_map.get(selected_code, {})
                    label = selected_code or label_default
                    validation_image = fetch_reference_image_from_url(color_table_validation_url(selected_row))
                    ...
                    if validation_image is None:
                        st.warning("该成品面料编号的颜色校验图暂时无法加载，请改用手动上传。")
                    if render_image is None:
                        st.warning("该成品面料编号的 HTML 颜色代码无效，请改用手动上传。")
                else:
